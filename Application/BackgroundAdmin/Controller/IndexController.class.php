...
class IndexController extends CommonController {
    public function index(){
        ...
        $this->assign('class_name',$pclass_name['pclass_name']);
    ...
    public function queryclass(){
        $pclass_id=$_SESSION['pclass_id'];
        $pclass_name=M('pclass')->where(array('pclass_id'=>$pclass_id))->field('pclass_name')->find();
        $grade=M('pclass')->where(array('pclass_id'=>$pclass_id))->find();
    ...
    public function auditscore(){
        if(IS_POST){
            ...
            $this->success('审核成功','queryclass/pclass_id/'.$pclassid,1);
        }else{
            $this->error("非法操作");
        }
    }
    public function change(){
        if(IS_POST){
            $usermsg=array('user_name'=>I('user_name'),
                'user_phone'=>I('user_phone')
                );
            $changere=M('user')->where(array('user_id'=>I('user_id')))->save($usermsg);
            $this->success('修改成功');
            return;
        }
        $user_id=I('get.user_id');
        $user=M('user')->where(array('user_id'=>$user_id))->field('user_id,user_name,user_phone')->find();
        $this->assign('user',$user);
        $this->display();
    }
}