...
class classController extends CommonController {
    ...
    public function out(){
        ...
        $instructor_id=$_SESSION['instructor_id'];
        $grade=M('instructor')->where(array('instructor_id'=>$instructor_id))->field('grade_id')->find();
        $grade_name=M('grade')->where(array('grade_id'=>$grade['grade_id']))->find();
    ...
    public function inputtable(){
        ...
        for($currentRow=1;$currentRow<=$allRow;$currentRow++){
            ...
            $nowyear=M('schoolyear')->where(array('schoolyear_nowyear'=>1))->find();
            $instructor_id=$_SESSION['instructor_id'];
            $grade=M('instructor')->where(array('instructor_id'=>$instructor_id))->find();
            for($j=2;$j<=$i;$j++){
                ...
                }else if($n['grade_id']!=$grade['grade_id']){
